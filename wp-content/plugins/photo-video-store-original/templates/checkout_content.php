...
<?php
	if ( $pvs_global_settings["no_cart"] and ! is_user_logged_in() ) {
		?>
		<script>
			function write_email(value)
			{
				$("#guest_email").val(value);
				
				reg = /.+@.+\..+/igm;
				
				if( value.match( reg ) ) 
				{
					$("#new_email").css("border-color","green");
				}
				else
				{
		...
		<div class="checkoutbox"><?php echo pvs_word_lang( "Create an account" )?></div>
			<div class="checkoutbox_text">
				...
				<input class="ibox form-control" type="text" id="new_email" value="" style="width:200px" onkeyup="write_email(this.value)">
				</div>
