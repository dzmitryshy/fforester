...
$sql = "select id, media_id, title, data, published, server1, server2, url_jpg, url_png, url_gif, url_raw, url_tiff, url_eps, url_jpf, url_jp2, url_zip from " .
...
while ( ! $rs->eof ) {
	...
	if ( $rs->row["media_id"] == 1 ) {
		...
		if ( $rs->row["url_jp2"] != "" )
		{
			$items_mass[$rs->row["url_jp2"]] = -1;
		}
		if ( $rs->row["url_jpf"] != "" )
		{
			$items_mass[$rs->row["url_jpf"]] = -1;
		}
		if ( $rs->row["url_zip"] != "" )
		{
			$items_mass[$rs->row["url_zip"]] = -1;
		}
	} else {
