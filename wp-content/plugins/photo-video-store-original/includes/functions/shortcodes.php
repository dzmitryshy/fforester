	...
	?>
		<script>
			function show_results_count() {
				$('#result_count').html('(<?php echo ( int )@$_SESSION["stock_result_count"] ?>)')
			}
			show_results_count()
		</script>
		<?php
...
function pvs_shortcodes_wrap( $atts, $type ) {
	...
	if ( $autopaging == 1 and $type == 'footer' ) {
		...
		$shortcodes_wrapper .= "$('.home_preview').each(function(){
									...
									req.open(null, '" . site_url() . "/content-list-paging/', true);
									req.send( {" . @$flow_vars . ",str: str,id_parent:" . (int)@$id_parent . "} );
									str++;
									}
