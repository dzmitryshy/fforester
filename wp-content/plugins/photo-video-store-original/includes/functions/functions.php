	...
	for ( $i = 0; $i < count( $pvs_menu_admin ); $i++ )
	{
		...
		foreach ( $pvs_submenu_admin as $key => $value )
		{
			if ( preg_match( "/^" . $pvs_menu_admin[$i] . "_/", $key ) )
			{
				...
				{
					$menu_count = '<span class="label label-danger pull-right">' . pvs_get_admin_notifications( 'orders', 'count' ) .
						'</span>';
				}
	
				if ( $key == "orders_credits" and pvs_get_admin_notifications( 'credits', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-warning pull-right">' . pvs_get_admin_notifications( 'credits', 'count' ) .
						'</span>';
				}
	
				if ( $key == "orders_subscription" and pvs_get_admin_notifications( 'subscription', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-info pull-right">' . pvs_get_admin_notifications( 'subscription', 'count' ) .
						'</span>';
				}
	
				if ( $key == "orders_commission" and pvs_get_admin_notifications( 'commission', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-primary pull-right">' . pvs_get_admin_notifications( 'commission', 'count' ) .
						'</span>';
				}
	
				if ( $key == "orders_downloads" and pvs_get_admin_notifications( 'downloads', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-success pull-right">' . pvs_get_admin_notifications( 'downloads', 'count' ) .
						'</span>';
				}
	
				if ( $key == "orders_invoices" and pvs_get_admin_notifications( 'invoices', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-info pull-right">' . pvs_get_admin_notifications( 'invoices', 'count' ) .
						'</span>';
				}
	
				if ( $key == "orders_payments" and pvs_get_admin_notifications( 'payments', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-warning pull-right">' . pvs_get_admin_notifications( 'payments', 'count' ) .
						'</span>';
				}
	
				if ( $key == "catalog_exam" and pvs_get_admin_notifications( 'exams', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-info pull-right">' . pvs_get_admin_notifications( 'exams', 'count' ) .
						'</span>';
				}
	
				if ( $key == "catalog_comments" and pvs_get_admin_notifications( 'comments', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-warning pull-right">' . pvs_get_admin_notifications( 'comments', 'count' ) .
						'</span>';
				}
	
				if ( $key == "catalog_lightboxes" and pvs_get_admin_notifications( 'lightboxes', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-success pull-right">' . pvs_get_admin_notifications( 'lightboxes', 'count' ) .
						'</span>';
				}
	
				if ( $key == "users_customers" and pvs_get_admin_notifications( 'users', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-danger pull-right">' . pvs_get_admin_notifications( 'users', 'count' ) .
						'</span>';
				}
	
				if ( $key == "users_documents" and pvs_get_admin_notifications( 'documents', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-success pull-right">' . pvs_get_admin_notifications( 'documents', 'count' ) .
						'</span>';
				}
	
				if ( $key == "users_messages" and pvs_get_admin_notifications( 'messages', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-warning pull-right">' . pvs_get_admin_notifications( 'messages', 'count' ) .
						'</span>';
				}
	
				if ( $key == "users_contacts" and pvs_get_admin_notifications( 'contacts', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-success pull-right">' . pvs_get_admin_notifications( 'contacts', 'count' ) .
						'</span>';
				}
	
				if ( $key == "users_testimonials" and pvs_get_admin_notifications( 'testimonials', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-info pull-right">' . pvs_get_admin_notifications( 'testimonials', 'count' ) .
						'</span>';
				}
	
				if ( $key == "users_support" and pvs_get_admin_notifications( 'support', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-danger pull-right">' . pvs_get_admin_notifications( 'support', 'count' ) .
						'</span>';
				}
				if ( $key == "catalog_upload" and pvs_get_admin_notifications( 'uploads', 'count' ) > 0 )
				{
					$menu_count = '<span class="label label-danger pull-right">' . pvs_get_admin_notifications( 'uploads', 'count' ) .
						'</span>';
				}
...
function pvs_load_js_css_admin()
{
	...
	/*
	wp_enqueue_script( 'pvs_jquery_admin', 'https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js' );
	wp_enqueue_script( 'pvs_jqueryui_admin', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js' );
	wp_enqueue_script( 'bootstrap_admin', '//maxcdn.bootstrapcdn.com/bootstrap/3.3.2/js/bootstrap.min.js' );
	wp_enqueue_script( 'pvs_colorbox_admin', pvs_plugins_url() . '/assets/js/jquery.colorbox-min.js' );
...
function pvs_load_js_css()
{
	...
	wp_enqueue_script( 'pvs_jquery', 'https://code.jquery.com/jquery-3.3.1.min.js' );
	...
	wp_enqueue_script( 'pvs_colorbox', 'https://cdnjs.cloudflare.com/ajax/libs/jquery.colorbox/1.6.4/jquery.colorbox-min.js' );
	...
	wp_enqueue_script( 'pvs_gallery', pvs_plugins_url() . '/includes/plugins/galleria/galleria-1.2.9.js' );
		...
		if ( $pvs_global_settings["lightbox_photo"] and ! preg_match( "/icon_photo/", $item_img_lightbox ) and ! preg_match( "/adult_content/", $item_img_lightbox ) )
		{
			if ( $hoverbox_results["width"] != 0 and $hoverbox_results["height"] != 0 )
			{
				...
				$hoverbox_results["hover"] = "onMouseover=\"lightboxon('" . $hoverbox_results["image"] .
					"');\" onMouseout=\"lightboxoff();\" onMousemove=\"lightboxmove(" .
...
/**
 * Create utility divs on the page. DEPRECATED. It will be removed in the new versions.
 *
 * @return string
 */
...
function pvs_build_variables( $var_default, $var_default2, $show_phpfile = true,
	...
	if ( $ajax == false ) {
		if ( $show_phpfile ) {
			$redirect_page = site_url() . "/?" . $result_vars;
			
			if ( $pvs_global_settings["catalog_url"] != '' ) {
				$redirect_page = $pvs_global_settings["catalog_url"] . "?" . $result_vars;
			}
