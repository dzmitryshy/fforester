...
function pvs_rights_managed_upload_form( $type, $rights_managed, $id, $admin_session )
{
	...
	function change_rights_upload_url(value)
	{
		if (value == 1) 
		{
			$(".rights_upload_area").css("display","none");
			$(".rights_url_area").css("display","block");
		}
		else
		{
			$(".rights_upload_area").css("display","block");
			$(".rights_url_area").css("display","none");
		}
	}	
	
	function check_remote_rights(form_field_id, form_field_class, file_format, text_area_id)
	{
		value = $("#"+form_field_id).val();
		$("."+form_field_class).not("#"+form_field_id).val(value);
				...
				success:function(data){
					...
					if (data == 1)
					{
						...
						if( result )
						{
							...
							{
								$("."+text_area_id).html("<span style=\"color:red\">' . pvs_word_lang('URL must start with http:// or https://') . '</span>");
							}
						}
						else
						{
							$("."+text_area_id).html("<span style=\"color:red\">' . pvs_word_lang('Incorrect file type') . '</span>");
						}
					}
					else
					{
						$("."+text_area_id).html("<span style=\"color:red\">' . pvs_word_lang('unable to open remote file') . '</span>");
					}
				}
			});
		}
...
function pvs_photo_upload_form( $id, $admin_session )
{
	...
	function change_file_upload_url(value)
	{
		if (value == 1) 
		{
			$(".file_upload_area").css("display","none");
			$(".file_url_area").css("display","block");
		}
		else
		{
			$(".file_upload_area").css("display","block");
			$(".file_url_area").css("display","none");
		}
	}	
...
function pvs_files_upload_form( $id, $type, $admin_session )
{
	...
	while ( ! $dr->eof )
	{
		...
		while ( ! $ds->eof )
		{
			...
			if ( $id != 0 )
			{
				$sql = "select id,price,url,shipped,name from " . PVS_DB_PREFIX .
				...
				if ( $type == "photo" )
				{
					...
					if ( $photo_file != "" and isset( $_POST[$type . "_chk" . $ds->row["id_parent"]] ) )
					{
						$sql = "insert into " . PVS_DB_PREFIX .
							...
							row["priority"] . ",0," . $ds->row["id_parent"] . "," . $ds->row["contacts"] . ")";
						$db->execute( $sql );
					}
				}
			}
...
function pvs_prints_upload_form()
{
	...
	{
		if( $('#prints_all').prop('checked') )
		{
			$('.prints_price').prop('checked', true); 
		}
		else
		{
			$('.prints_price').prop('checked', false); 
		}
	}</script>
...
function pvs_get_preview_form( $type, $flag_rights_managed, $id = 0 )
{
	...
	if ( $flag_storage ) {
		...
		if ( ! $dp->eof ) {
			if ( $type == 'video' )
			{
				$previews_remote['video_video'] = pvs_show_preview( $id, $type, 2, 1, $dp->row["server1"], $id );
	...
	function check_youtube(form_field_id, form_field_class)
	{
		...
		if(video_hosting == "youtube")
		{
			...
			$(".photo-preview-field").val(photo_preview);
			$(".preview_youtube").css("display","inline");
			$(".preview_vimeo").css("display","none");
			$(".upload_photo_area").html("<img src=\""+photo_preview+"\" style=\"max-width:120px;margin-top:3px\">");
			$(".upload_video_area").html("<iframe width=\"' . $pvs_global_settings["ffmpeg_video_width"] . '\" height=\"' . $pvs_global_settings["ffmpeg_video_height"] . '\" src=\"https://www.youtube.com/embed/"+video_id+"\" frameborder=\"0\" allow=\"autoplay; encrypted-media\" allowfullscreen></iframe>");
		}
		...
		if(video_hosting == "vimeo")
		{
			...
			$.getJSON("http://www.vimeo.com/api/v2/video/" + video_id + ".json?callback=?", { format: "json" }, function (data) {
					...
					$(".preview_vimeo").css("display","inline");
					$(".upload_photo_area").html("<img src=\""+photo_preview+"\" style=\"max-width:120px;margin-top:3px\">");
                });
                
			$(".upload_video_area").html("<iframe src=\"https://player.vimeo.com/video/"+video_id+"?autoplay=0&loop=0\" width=\"' . $pvs_global_settings["ffmpeg_video_width"] . '\" height=\"' . $pvs_global_settings["ffmpeg_video_height"] . '\" frameborder=\"0\" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>");
		}
		...
		if (video_check == false) {
			...
			$(".photo-preview-field").val("");
			$(".preview_youtube").css("display","none");
			$(".preview_vimeo").css("display","none");
			$(".upload_photo_area").html("");
			$(".upload_video_area").html("");
		}
