...
(function ($) {
	...
	(function(){
		...
		function splitEvents(events, id) {
			...
			var ret = {d: [], w: []};
			jQuery.each((events || '').split(' '), function(k, v){
				var eventname = v + '.' + id;
		...
		mejs.MediaElementPlayer.prototype.globalBind = function(events, data, callback) {
			...
			if (events.d) jQuery(document).bind(events.d, data, callback);
			if (events.w) jQuery(window).bind(events.w, data, callback);
		};

		mejs.MediaElementPlayer.prototype.globalUnbind = function(events, callback) {
			var t = this;
			events = splitEvents(events, t.id);
			if (events.d) jQuery(document).unbind(events.d, callback);
			if (events.w) jQuery(window).unbind(events.w, callback);
		};
	})();
...
(function($) {
	...
	jQuery.extend(MediaElementPlayer.prototype, {
					...
					if (supportsPointerEvents && !mejs.MediaFeatures.isOpera) { // opera doesn't allow this :(
						...
						*/
						...
						for (i = 0, len = hoverDivNames.length; i < len; i++) {
							hoverDivs[hoverDivNames[i]] = jQuery('<div class="mejs-fullscreen-hover" />').appendTo(t.container).mouseover(restoreControls).hide();
						}
		...
		enterFullScreen: function() {
            ...
            jQuery(document.documentElement).addClass('mejs-fullscreen');
		...
		exitFullScreen: function() {
            ...
            jQuery(document.documentElement).removeClass('mejs-fullscreen');
...
(function($) {
	...
	jQuery.extend(MediaElementPlayer.prototype, {
		...
		buildtracks: function(player, controls, layers, media) {
			...
			player.captions = 
					jQuery('<div class="mejs-captions-layer mejs-layer"><div class="mejs-captions-position mejs-captions-position-hover"><span class="mejs-captions-text"></span></div></div>')
						.prependTo(layers).hide();
			...
			if (player.options.slidesSelector != '') {
				player.slidesContainer = jQuery(player.options.slidesSelector);
		...
		showSlide: function(index) {
			...
			if (typeof img == 'undefined' || typeof img.fadeIn == 'undefined') {
				...
				t.slides.entries.imgs[index] = img = jQuery('<img src="' + url + '">')
						.on('load', function() {
	...
	jQuery.extend(MediaElementPlayer.prototype, {
		buildpostroll: function(player, controls, layers, media) {
			...
			if (typeof postrollLink !== 'undefined') {
				player.postroll =
					jQuery('<div class="mejs-postroll-layer mejs-layer"><a class="mejs-postroll-close" onclick="jQuery(this).parent().hide();return false;">' + t.options.postrollCloseText + '</a><div class="mejs-postroll-layer-content"></div></div>').prependTo(layers).hide();
