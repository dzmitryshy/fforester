...
$keyword_url = pvs_catalog_url( true );
...
if ( (int) get_query_var('shutterstock') > 0 ) {
    ...
    if ( ! is_wp_error( $result ) ) {
        $shutterstock_results = json_decode( $result["body"] );
        $flag_stock           = true;
        ...
        if ( isset( $shutterstock_results->keywords ) ) {
            foreach ( $shutterstock_results->keywords as $key => $value ) {
                ...
                $shutterstock_keywords_links .= '<a href="' . $keyword_url . '?stock=shutterstock&search=' . urlencode( $value ) . $var_print . '" >' . $value . '</a>';
            }
        }
        ...
        if ( isset( $shutterstock_results->categories ) ) {
            foreach ( $shutterstock_results->categories as $key => $value ) {
                ...
                $shutterstock_categories_links .= '<a href="' . $keyword_url . '?stock=shutterstock&category=' . @$value->id . $var_print . '" >' . @$value->name . '</a>';
            }
        }
...
if ( (int) get_query_var('fotolia') > 0 ) {
    ...
    if ( ! is_wp_error( $result ) ) {
        $fotolia_results = json_decode( $result["body"] );
        $flag_stock      = true;
        ...
        if ( isset( $fotolia_results->keywords ) ) {
            foreach ( $fotolia_results->keywords as $key => $value ) {
                ...
                $fotolia_keywords_links .= '<a href="' . $keyword_url . '?stock=fotolia&search=' . urlencode( $value->name ) . $var_print . '" >' . $value->name . '</a>';
            }
        }
        ...
        if ( isset( $fotolia_results->cat1_hierarchy ) ) {
            foreach ( $fotolia_results->cat1_hierarchy as $key => $value ) {
                ...
                $fotolia_categories_links .= '<a href="' . $keyword_url . '?stock=fotolia&category=' . @$value->id . $var_print . '" >' . @$value->name . '</a>';
            }
        }
            ...
            if ( isset( $istockphoto_results->keywords ) ) {
                ...
                foreach ( $istockphoto_results->keywords as $key2 => $value2 ) {
                    if ( isset( $value2->text ) ) {
                        ...
                        $istockphoto_keywords_links .= '<a href="' . $keyword_url . '?stock=istockphoto&search=' . urlencode( @$value2->text ) . $var_print . '" >' . @$value2->text . '</a>';
                        if ( $kk == 0 ) {
...
if ( (int) get_query_var('depositphotos') > 0 ) {
	...
	if ( ! is_wp_error( $result ) ) {
        $depositphotos_results = json_decode( $result["body"] );
        $flag_stock            = true;
        ...
        if ( isset( $depositphotos_results->tags ) ) {
            foreach ( $depositphotos_results->tags as $key => $value ) {
                ...
                $depositphotos_keywords_links .= '<a href="' . $keyword_url . '?stock=depositphotos&search=' . urlencode( $value ) . $var_print . '" >' . $value . '</a>';
            }
        }
        ...
        if ( isset( $depositphotos_results->categories ) ) {
            foreach ( $depositphotos_results->categories as $key => $value ) {
                ...
                $depositphotos_categories_links .= '<a href="' . $keyword_url. '?stock=depositphotos&category=' . @$key . $var_print . '" >' . @$value . '</a>';
            }
        }
...
if ( (int) get_query_var('bigstockphoto') > 0 ) {
    ...
    if ( ! is_wp_error( $result ) ) {
        $bigstockphoto_results2 = json_decode( $result["body"] );
        ...
        foreach ( $kw as $key => $value ) {
            ...
            $bigstockphoto_keywords_links .= '<a href="' . $keyword_url . '?stock=bigstockphoto&search=' . urlencode( $value ) . $var_print . '" >' . $value . '</a>';
        }
        ...
        if ( isset( $bigstockphoto_results->categories ) ) {
            foreach ( $bigstockphoto_results->categories as $key => $value ) {
                ...
                $bigstockphoto_categories_links .= '<a href="' . $keyword_url . '?stock=bigstockphoto&category=' . urlencode( @$value->name ) . $var_print . '" >' . @$value->name . '</a>';
            }
        }
...
if ( (int) get_query_var('rf123') > 0 ) {
    ...
    if ( ! is_wp_error( $result ) ) {
        $rf123_results2 = json_decode( json_encode( simplexml_load_string( $result["body"] ) ) );
        ...
        foreach ( $rf123_results->keywords->keyword as $key => $value ) {
            ...
            $rf123_keywords_links .= '<a href="' . $keyword_url . '?stock=rf123&search=' . urlencode( $value ) . $var_print . '" >' . $value . '</a>';
...
if ( (int) get_query_var('pixabay') > 0 ) {
	...
	if ( ! is_wp_error( $result ) ) {
        $pixabay_results2 = json_decode( $result["body"] );
        $flag_stock       = true;
        ...
        foreach ( $pixabay_results2->hits as $key => $value ) {
            ...
            if ( isset( $value->tags ) ) {
                ...
                foreach ( $pixabay_kw as $key2 => $value2 ) {
                    ...
                    $pixabay_keywords_links .= '<a href="' . $keyword_url . '?stock=pixabay&search=' . urlencode( $value2 ) . $var_print . '" >' . $value2 . '</a>';
                }
            }
...
if ( (int) get_query_var('adobe') > 0 ) {
    ...
    $data = curl_exec( $ch );
    if ( !curl_errno( $ch ) ) {
        $adobe_results2 = json_decode( $data );
        $flag_stock       = true;
        
        foreach ( $adobe_results2->files as $key => $value ) {
            $adobe_results        = @$value;
            $adobe_keywords       = "";
            $adobe_keywords_links = "";
            if ( isset( $value->keywords ) ) {
                foreach ( $value->keywords as $key2 => $value2 ) {
                    if ( $adobe_keywords != "" ) {
                        $adobe_keywords .= ', ';
                    }
                    if ( $adobe_keywords_links != "" ) {
                        $adobe_keywords_links .= ', ';
                    }
                    $adobe_keywords .= $value2->name;
                    $adobe_keywords_links .= '<a href="' . $keyword_url . '?stock=adobe&search=' . urlencode( $value2->name ) . $var_print . '" >' . $value2->name . '</a>';
                }
            }

            $adobe_categories = "";
            $adobe_categories_links = "";
            
            if ( isset( $value->category ) ) {				
					$adobe_categories .= @$value->category->name;
					$adobe_categories_links .= '<a href="' . $keyword_url . '?stock=adobe&category=' . (int) @$value->category->id  . $var_print . '" >' . @$value->category->name . '</a>';
			}
            
            $meta_title = @$value->title;
            $pvs_meta_keywords .= $adobe_keywords;
            $pvs_meta_description .= @$value->description;
            $social_mass[ "type" ]        = @$value->media_type_id;
            $social_mass[ "title" ]       = @$value->title;
            $social_mass[ "keywords" ]    = $adobe_keywords;
            $social_mass[ "description" ] = @$value->description;
            
            if ( $prints_flag ) {
                $social_mass[ "url" ] = site_url() . pvs_print_url( @$value->id, (int) get_query_var('pvs_print_id'), @$value->title, $prints_preview, "adobe" );
            } else {
                $social_mass[ "url" ] = site_url() . pvs_get_stock_page_url( "adobe", @$value->id, @$value->title, pvs_result( get_query_var('adobe_type') ) );
            }
            $social_mass[ "author" ]   = "Adobe Contributor: " . @$value->creator_name;
            $social_mass[ "google_x" ] = "";
            $social_mass[ "google_y" ] = "";
            $social_mass[ "data" ]     = "";
            
            $social_mass[ "image" ] = @$value->thumbnail_url;
            
            $social_mass[ "category" ] = $adobe_categories;
        }
    }
}
//End. Adobe API
