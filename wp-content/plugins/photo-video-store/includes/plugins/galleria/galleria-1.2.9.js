    ...
    _video = {
        youtube: {
            ...
            getThumb: function( id, success, fail ) {
                fail = fail || F;
                jQuery.getJSON(window.location.protocol+'//gdata.youtube.com/feeds/api/videos/' + id + '?v=2&alt=json-in-script&callback=?', function(data) {
                    try {
        ...
        vimeo: {
            ...
            getThumb: function( id, success, fail ) {
                fail = fail || F;
                jQuery.getJSON('http://vimeo.com/api/v2/video/' + id + '.json?callback=?', function(data) {
                    try {
        ...
        dailymotion: {
            ...
            getThumb: function( id, success, fail ) {
                fail = fail || F;
                jQuery.getJSON('https://api.dailymotion.com/video/'+id+'?fields=thumbnail_medium_url&callback=?', function(data) {
                    try {
    ...
    Utils = (function() {
        ...
        return {
            ...
            clearTimer: function( id ) {
                jQuery.each( Galleria.get(), function() {
                    this.clearTimer( id );
                });
            },
            ...
            loadCSS : function( href, id, callback ) {
                ...
                if ( typeof callback === 'function' ) {
                    ...
                    var $loader = jQuery('<s>').attr( 'id', 'galleria-loader' ).hide().appendTo( DOM().body );
...
Galleria = function() {
        ...
        parseCallback: function( callback, enter ) {
            ...
            return _transitions.active ? function() {
                ...
                if ( enter && self._options.trueFullscreen ) {
                    ...
                    jQuery( active.container ).add( next.container ).trigger( 'transitionend' );
                }
            ...
            if ( self._options.trueFullscreen && _nativeFullscreen.support ) {
                ...
                if ( Galleria.MAC ) {
                    if ( Galleria.WEBKIT && !( Galleria.SAFARI && /version\/[1-5]/.test(NAV)) ) {
                        self.jQuery('container').css('opacity', 0).addClass('fullscreen');
                        window.setTimeout(function() {
                            fullscreen.scale();
                            self.jQuery('container').css('opacity', 1);
                        }, 50);
                    } else {
                        self.jQuery('stage').css('opacity', 0);
                        window.setTimeout(function() {
                            fullscreen.scale();
                            self.jQuery('stage').css('opacity', 1);
                        },4);
                    }
                } else {
        ...
        _enter: function( callback ) {
            ...
            if ( IFRAME ) {
                ...
                fullscreen.iframe = (function() {
                    ...
                    jQuery( fullscreen.pd ).find('iframe').each(function() {
            ...
            if ( IFRAME && fullscreen.iframe ) {
                fullscreen.iframe.scrolled = jQuery( window.parent ).scrollTop();
            ...
            if ( IFRAME && fullscreen.iframe ) {
                ...
                Utils.forceStyles( fullscreen.iframe, jQuery.extend( htmlbody, {
                    width: '100%',
                    height: '100%',
                    top: 0,
                    left: 0,
    ...
    var idle = this._idle = {
        ...
        add: function(elem, to, from, hide) {
            ...
            elem.data('idle', {
                from: jQuery.extend( extract, from ),
                to: to,
                complete: true,
                busy: false
            });
        ...
        addEvent : function() {
            ...
            if ( self._options.idleMode == 'hover' ) {
                self.jQuery('container').bind( 'mouseleave', idle.hide );
            }
        },
        ...
        removeEvent : function() {
            ...
            self.jQuery('container').bind( 'mousemove click', idle.showAll );
            if ( self._options.idleMode == 'hover' ) {
                self.jQuery('container').unbind( 'mouseleave', idle.hide );
            }
        },
        ...
        show: function(elem) {
            ...
            if ( !idle.active || ( !data.busy && !data.complete ) ) {
                ...
                Utils.animate( elem, data.from, {
                    ...
                    complete: function() {
                        idle.active = true;
                        jQuery(elem).data('idle').busy = false;
                        jQuery(elem).data('idle').complete = true;
                    }
                });
    ...
    var lightbox = this._lightbox = {
            ...
            if ( event ) {
                ...
                jQuery( lightbox.elems.box ).animate( to, {
                    ...
                    complete: function() {
                        ...
                        jQuery( image.image ).animate({ opacity: 1 }, speed);
        ...
        show: function(index) {
            ...
            jQuery(lightbox.elems.box).toggleClass( 'iframe', !!data.iframe );
            ...
            jQuery( lightbox.elems.overlay ).show().css( 'visibility', 'visible' );
...
Galleria.prototype = {
    ...
    */
    ...
    _init: function() {
        ...
        jQuery.extend( true, options, Galleria.theme.defaults, this._original.options, Galleria.configure.options );
        ...
        // bind the ons
        jQuery.each( Galleria.on.binds, function(i, bind) {
            // check if already bound
            if ( jQuery.inArray( bind.hash, self._binds ) == -1 ) {
    ...
    _createThumbnails : function( push ) {
        ...
        var src,
            ...
            thumbComplete = function( thumb, callback ) {
                ...
                jQuery( thumb.container ).css( 'visibility', 'visible' );
                self.trigger({
                    type: Galleria.THUMBNAIL,
                ...
                thumb.scale({
                    ...
                    complete: function( thumb ) {
                        ...
                        if( data.iframe && special.length == 2 && special[0] in _video ) {
                            ...
                            jQuery.each( thumbchunk, function( i, th ) {
        ...
        for( ; this._data[ i ]; i++ ) {
            ...
            // create empty spans if thumbnails is set to 'empty'
                ...
                if ( data.iframe ) {
                    jQuery( thumb.image ).addClass( 'iframe' );
                }
    ...
    */
    ...
    lazyLoad: function( index, complete ) {
        ...
        var arr = index.constructor == Array ? index : [ index ],
            self = this,
            thumbnails = this.jQuery( 'thumbnails' ).children().filter(function() {
                return jQuery(this).data('lazy-src');
            }),
            loaded = 0;

        jQuery.each( arr, function(i, ind) {
            ...
            var thumb = self._thumbnails[ ind ],
                ...
                thumbload = jQuery( thumb.container ).data( 'thumbload' );
            if ( thumb.video ) {
    ...
    _run : function() {
        ...
        Utils.wait({
            ...
            success: function() {
                ...
                jQuery.each( Galleria.ready.callbacks, function(i ,fn) {
    ...
    */
    ...
    load : function( source, selector, config ) {
        ...
        if ( source.constructor === Array ) {
            ...
            jQuery( source ).find( selector ).each( function( i, elem ) {
                ...
                jQuery.each( 'big title description link layer'.split(' '), function( i, val ) {
                    if ( elem.data(val) ) {
                ...
                if ( result ) {
                    current.iframe = _video[ result.provider ].embed( result.id ) + (function() {
                        ...
                        if ( typeof self._options[ result.provider ] == 'object' ) {
                            var str = '?', arr = [];
                            jQuery.each( self._options[ result.provider ], function( key, val ) {
                                arr.push( key + '=' + val );
                            });
    ...
    */
    ...
    destroy : function() {
        this.jQuery( 'target' ).data( 'galleria', null );
        this.jQuery( 'container' ).unbind( 'galleria' );
...
this.prependChild( 'info', 'myElement' );
    ...
    */
    ...
    resize : function( measures, complete ) {
        ...
        measures = jQuery.extend( { width:0, height:0 }, measures );

        var self = this,
            $container = this.jQuery( 'container' );

        jQuery.each( measures, function( m, val ) {
            if ( !val ) {
                $container[ m ]( 'auto' );
    ...
    _show : function() {
        ...
        var src = data.iframe || ( this.isFullscreen() && 'big' in data ? data.big : data.image ), // use big image if fullscreen mode
            ...
            mousetrigger = function() {
                jQuery( next.image ).trigger( 'mouseup' );
            };
        ...
        var complete = (function( data, next, active, queue, thumb ) {
            ...
            return function() {
                ...
                if( active.isIframe ) {
                    jQuery( active.container ).find( 'iframe' ).remove();
                }

                self.jQuery('container').toggleClass('iframe', !!data.iframe);
        ...
        next.load( src, function( next ) {
            ...
            // add layer HTML
            var layer = jQuery( self._layers[ 1-self._controls.active ] ).html( data.layer || '' ).hide();
...
Galleria.loadTheme = function( src, options ) {
    ...
    if( jQuery('script').filter(function() { return jQuery(this).attr('src') == src; }).length ) {
        return;
    }
...
/**
    ...
    Run Galleria
    Alias for jQuery(selector).galleria(options)
...
Galleria.run = function( selector, options ) {
    ...
    jQuery( selector || '#galleria' ).galleria( options );
    return Galleria;
};
        ...
        echo = function( msg ) {
            ...
            jQuery.each( _instances, function() {
                ...
                if ( !cont.length ) {
                    ...
                    cont = this.addElement( 'errors' ).appendChild( 'target', 'errors' ).jQuery( 'errors' ).css(css);
                }
                cont.append( html );
            ...
            if ( !_instances.length ) {
                jQuery('<div>').css( jQuery.extend( css, { position: 'fixed' } ) ).append( html ).appendTo( DOM().body );
            }
        };
    ...
    */
    ...
    load: function(src, size, callback) {
        ...
        if( this.isIframe ) {
            ...
            jQuery( this.container ).find( 'iframe,img' ).remove();

            this.container.appendChild( this.image );

            jQuery('#'+id).load( (function( self, callback ) {
                return function() {
                    window.setTimeout(function() {
                        jQuery( self.image ).css( 'visibility', 'visible' );
                        if( typeof callback == 'function' ) {
        ...
        if ( Galleria.IE8 ) {
            jQuery( this.image ).css( 'filter', 'inherit' );
        }
                        ...
                        window.setTimeout( (function( img ) {
                            return function() {
                                if ( img.width && img.height ) {
                                    ...
                                    if ( !resort ) {
                                        jQuery(new Image()).load( onload ).attr( 'src', img.src );
                                        resort = true;
                                    } else {
        ...
        jQuery.each('minWidth minHeight maxWidth maxHeight'.split(' '), function(i, prop) {
    ...
    */
    ...
    scale: function( options ) {
        ...
        if( this.isIframe ) {
            ...
            jQuery( this.image ).width( nw || cw ).height( nh || ch ).removeAttr( 'width' ).removeAttr( 'height' );
            jQuery( this.container ).width( cw ).height( ch );
            options.complete.call(self, self);
            try {
                if( this.image.contentWindow ) {
                    jQuery( this.image.contentWindow ).trigger('resize');
                }
            } catch(e) {}
            return this.container;
...
jQuery.fn.galleria = function( options ) {
    ...
    if ( !jQuery(this).length ) {
        ...
        jQuery(function() {
            if ( jQuery( selector ).length ) {
                ...
                jQuery( selector ).galleria( options );
    ...
    return this.each(function() {
        ...
        if ( jQuery.data(this, 'galleria') ) {
            jQuery.data( this, 'galleria' ).destroy();
            jQuery( this ).find( '*' ).hide();
        }

        // load the new gallery
        jQuery.data( this, 'galleria', new Galleria().init( this, options ) );
    });
