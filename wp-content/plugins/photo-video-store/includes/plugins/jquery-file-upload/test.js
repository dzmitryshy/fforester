...
jQuery(function () {
    ...
    QUnit.done = function () {
        ...
        var url = jQuery('#fileupload').prop('action');
    ...
    var lifecycle = {
            setup: function () {
                ...
                jQuery.widget('blueimp.fileupload', window.testBasicWidget, {});
            },
            teardown: function () {
        ...
        lifecycleUI = {
            setup: function () {
                ...
                jQuery.widget('blueimp.fileupload', window.testUIWidget, {});
            },
            teardown: function () {
                // Remove all remaining event listeners:
                jQuery(document).unbind();
    ...
    test('Data attribute options', function () {
        jQuery('#fileupload').attr('data-url', 'http://example.org');
        jQuery('#fileupload').fileupload();
        strictEqual(
            jQuery('#fileupload').fileupload('option', 'url'),
            'http://example.org'
        );
    });
    ...
    test('Drop zone initialization', function () {
        ok(jQuery('#fileupload').fileupload()
    ...
    test('Event listeners initialization', function () {
        expect(
            jQuery.support.xhrFormDataFileUpload ? 4 : 1
        );
        var eo = {originalEvent: {}},
            fu = jQuery('#fileupload').fileupload({
                dragover: function () {
        ...
        fileInput.trigger(jQuery.Event('change', eo));
        dropZone.trigger(jQuery.Event('dragover', eo));
        dropZone.trigger(jQuery.Event('drop', eo));
        pasteZone.trigger(jQuery.Event('paste', eo));
    });
    ...
    test('destroy', function () {
        ...
        var eo = {originalEvent: {}},
            ...
            fu = jQuery('#fileupload').fileupload(options),
            fileInput = fu.fileupload('option', 'fileInput'),
            dropZone = fu.fileupload('option', 'dropZone'),
            pasteZone = fu.fileupload('option', 'pasteZone');
    ...
    test('option', function () {
        expect(
            jQuery.support.xhrFormDataFileUpload ? 10 : 7
        );
        var eo = {originalEvent: {}},
            fu = jQuery('#fileupload').fileupload({
                dragover: function () {
        ...
        fu.fileupload('option', 'fileInput', jQuery(':file')[0]);
        strictEqual(
            fu.fileupload('option', 'fileInput')[0],
            jQuery(':file')[0],
    ...
    test('url', function () {
        ...
        jQuery('#fileupload').fileupload({
            ...
            send: function (e, data) {
                strictEqual(
                    data.url,
                    jQuery(data.fileInput.prop('form')).prop('action'),
    ...
    test('Widget initialization', function () {
        ...
        ok(fu.data('blueimp-fileupload') || fu.data('fileupload'));
        ok(
            jQuery('#fileupload').fileupload('option', 'uploadTemplate').length,
            'Initialized upload template'
        );
        ok(
            jQuery('#fileupload').fileupload('option', 'downloadTemplate').length,
            'Initialized download template'
        );
    });

    test('Buttonbar event listeners', function () {
        var buttonbar = jQuery('#fileupload .fileupload-buttonbar'),
            files = [{name: 'test'}];
        expect(4);
        jQuery('#fileupload').fileupload({
            send: function (e, data) {
        ...
        jQuery('#fileupload').fileupload('add', {files: files});
        buttonbar.find('.cancel').click();
        jQuery('#fileupload').fileupload('add', {files: files});
        buttonbar.find('.start').click();
        buttonbar.find('.cancel').click();
        (jQuery('#fileupload').data('blueimp-fileupload') ||
                jQuery('#fileupload').data('fileupload'))
            ._renderDownload(files)
            .appendTo(jQuery('#fileupload .files')).show()
    ...
    test('destroy', function () {
        ...
        (jQuery('#fileupload').data('blueimp-fileupload') ||
            ...
            .show()
            .find('.delete input').click();
        jQuery('#fileupload .fileupload-buttonbar .delete').click();
    });
    ...
    test('maxNumberOfFiles', function () {
        ...
        jQuery('#fileupload')
            ...
            .fileupload({
                ...
                send: function (e, data) {
                    ok(
                        !jQuery.blueimp.fileupload.prototype.options
    ...
    test('acceptFileTypes as HTML5 data attribute', function () {
        ...
        jQuery('#fileupload')
            ...
            .fileupload();
        strictEqual(
            jQuery.type(jQuery('#fileupload').fileupload('option', 'acceptFileTypes')),
            jQuery.type(regExp)
        );
        strictEqual(
            jQuery('#fileupload').fileupload('option', 'acceptFileTypes').toString(),
            regExp.toString()
        );
    });
