...
//Delete
if ( @$_REQUEST["action"] == 'delete' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-downloads-delete' ) )
{
	include ( "delete.php" );
}

//Restore
if ( @$_REQUEST["action"] == 'restore' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-downloads-restore' ) )
{
	include ( "restore.php" );
}
...
<?php wp_nonce_field( 'pvs-downloads-delete' ); ?>
...
<?php
	$property_value = pvs_get_admin_notifications( 'downloads', 'value' );
	while ( ! $rs->eof ) {
		$cl3 = "";
		$cl_script = "";
	...
	?>
				...
				?>" style="max-width:<?php echo($pvs_global_settings["thumb_width"]);?>px"></a> </td>
...
?></span></td>
			...
			if ( $rs->row["tlimit"] > $rs->row["ulimit"] or $rs->row["data"] < pvs_get_time
				...
				echo ( pvs_word_lang( "expired" ) . " - <a href='" . pvs_plugins_admin_url('downloads/index.php') . "&action=restore&id=" . $rs->row["id"] .
					"&_wpnonce=" . wp_create_nonce('pvs-downloads-restore') . "'>" . pvs_word_lang( "restore link" ) . "</a>" );
			} else {
	?>
							<a href="<?php
	...
	?>
				...
				<?php
					...
					<td><a href="<?php echo(pvs_plugins_admin_url('collections/index.php'));?>&action=content&id=<?php echo $rs->row["collection_id"] ?>"><b><?php echo pvs_word_lang("Collection"); ?>: <?php echo $dd->row["title"]; ?> (<?php echo pvs_count_files_in_collection($dd->row["id"]); ?>)</b></a> <?php echo $cl3 ?></td>
					
					<td><?php echo date( date_format, ( $rs->row["data"] - $pvs_global_settings["download_expiration"] * 3600 * 24 ) )?></td>
					<td>
						<?php
							if ( $rs->row["order_id"] != 0 ) {
								echo ( "<a href='" . pvs_plugins_admin_url('orders/index.php') . "&action=order_content&id=" . $rs->row["order_id"] . "'>" .
					...
					<td><div class="link_user"><a href="<?php echo ( pvs_plugins_admin_url( 'customers/index.php' ) ); ?>&action=content&id=<?php echo $rs->row["user_id"]?>"><?php echo pvs_user_id_to_login( $rs->row["user_id"] )?></a></div></td>
					<td><span class="price"><?php echo pvs_currency( 1, true ); ?><?php echo pvs_price_format( $dd->row["price"], 2 )?> <?php echo pvs_currency( 2, true ); ?></span></td>
					<td>
						<?php
							if ( $rs->row["tlimit"] > $rs->row["ulimit"] or $rs->row["data"] < pvs_get_time
								( date( "H" ), date( "i" ), date( "s" ), date( "m" ), date( "d" ), date( "Y" ) ) ) {
								...
								?>
								<a href="<?php echo site_url()?>/download/?f=<?php echo $rs->row["link"] ?>"><b><?php echo pvs_word_lang( "link" )?></b></a><br>
								<?php echo pvs_word_lang( "downloads" )?>: <?php echo $rs->row["tlimit"] ?>(<?php echo $rs->row["ulimit"] ?>)
								<?php
							}
						?>
					</td>
				...
				if ( ! $dd->eof ) {
					...
					<tr valign="top" <?php echo $cl_script ?>>
					<td><input type="checkbox" name="sel<?php echo $rs->row["id"] ?>" id="sel<?php echo $rs->row["id"] ?>"></td>
					<td><a href="<?php echo(pvs_plugins_admin_url('packages/index.php'));?>&action=content&id=<?php echo $rs->row["package_id"] ?>"><img src="<?php echo $package_result["photo"]; ?>" style="max-width:<?php echo $pvs_global_settings["thumb_width"]; ?>px;max-height:<?php echo $pvs_global_settings["thumb_width"]; ?>px"></a></td>
					<td><a href="<?php echo(pvs_plugins_admin_url('packages/index.php'));?>&action=content&id=<?php echo $rs->row["package_id"] ?>"><b><?php echo pvs_word_lang("Package"); ?>: <?php echo $dd->row["title"]; ?></b></a> <?php echo $cl3 ?></td>
