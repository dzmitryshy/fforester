...
$afiles=pvs_parcer_csv(pvs_upload_dir().$pvs_global_settings["vectorpreupload"]."/vector.csv");
...
for ( $j = 1; $j < count( $afiles ); $j++ ) {
	...
	$csv_title = @$afiles[$j][2];
	$csv_description = @$afiles[$j][3];
	$csv_keywords = @$afiles[$j][4];
	$csv_author = @$afiles[$j][5];
	$csv_categories = @$afiles[$j][6];
	
	
	if ( file_exists ( pvs_upload_dir().$pvs_global_settings["vectorpreupload"]."/" . $csv_path ) ) {
		...
		$previewphoto = $pvs_global_settings["vectorpreupload"] . $csv_photo_preview;
		...
		while ( ! $ds->eof )
		{
			if ( $ds->row["shipped"] != 1 )
			{
				...
				if ( $flag )
				{
					$vector = $pvs_global_settings["vectorpreupload"] . $csv_path;
					copy( pvs_upload_dir() . $vector, pvs_upload_dir() . $site_servers[$site_server_activ] . "/" . $folder . "/" . $csv_filename );
					...
					if ( $ds->row["thesame"] > 0 )
					{
						$sql = "select * from " . PVS_DB_PREFIX . "vector_types where id_parent<>" . $ds->
		...
		if ( file_exists ( pvs_upload_dir() . $previewphoto ) ) {
			...
			copy( pvs_upload_dir() . $previewphoto, pvs_upload_dir() . $site_servers[$site_server_activ] . "/" . $folder . "/thumb_original.jpg" );
		}
