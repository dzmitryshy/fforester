...
//Add
if ( @$_REQUEST["action"] == 'add' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-prints-categories-add' ) )
{
	include ( "add.php" );
}

//Change
if ( @$_REQUEST["action"] == 'change' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-prints-categories-change' ) )
{
	include ( "change.php" );
}

//Delete
if ( @$_REQUEST["action"] == 'delete' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-prints-categories-delete' . $_GET["id"] ) )
{
	include ( "delete.php" );
}
?>
...
?>&action=add&_wpnonce=<?php echo wp_create_nonce('pvs-prints-categories-add'); ?>"><i class="icon-print icon-white fa fa-plus"></i>&nbsp; <?php
...
?>
	...
	<?php wp_nonce_field( 'pvs-prints-categories-change' ); ?>
...
?>&action=delete&id=<?php
		echo $rs->row["id"]
?>&_wpnonce=<?php echo wp_create_nonce('pvs-prints-categories-delete' . $rs->row["id"]); ?>'><?php
