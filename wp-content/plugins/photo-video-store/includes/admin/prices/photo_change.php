	...
	while ( ! $rs->eof )
	{
		...
		while ( ! $dr->eof )
		{
			$sql = "update " . PVS_DB_PREFIX . "sizes  set title='" . pvs_result( $_POST["title" .
				...
				$dr->row["id_parent"]] . ",jp2=" . ( int )@$_POST["jp2" .
				$dr->row["id_parent"]] . ",jpf=" . ( int )@$_POST["jpf" .
				$dr->row["id_parent"]] . ",zip=" . ( int )@$_POST["zip" .
				$dr->row["id_parent"]] . ",contacts=" . ( int )@$_POST["contacts" .
			...
			if ( ( int )$_POST["addto"] == 1 )
			{
				...
				$sql = "update " . PVS_DB_PREFIX . "items  set name='" . pvs_result( $_POST["title" .
					...
					@$_POST["watermark" . $dr->row["id_parent"]] . ",contacts=" . ( int )
					@$_POST["contacts" . $dr->row["id_parent"]] . " where price_id=" . $dr->row["id_parent"];
				$db->execute( $sql );
