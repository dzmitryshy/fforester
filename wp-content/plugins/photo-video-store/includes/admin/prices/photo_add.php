...
if ( wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-prices-add' ) ) {
	$sql = "insert into " . PVS_DB_PREFIX .
		"sizes (size,price,priority,title,license,watermark,jpg,png,gif,raw,tiff,eps,jp2,jpf,zip,contacts) values (" . ( int )
		...
		"," . ( int )@$_POST["eps"] . "," . ( int )@$_POST["jp2"] . "," . ( int )@$_POST["jpf"] . "," . ( int )@$_POST["zip"] . "," . ( int )@$_POST["contacts"] . ")";
	$db->execute( $sql );
	...
	if ( ( int )$_POST["addto"] == 1 )
	{
		...
		while ( ! $rs->eof )
		{
			...
			{
				$sql = "insert into " . PVS_DB_PREFIX .
					"items (id_parent,name,url,price,priority,shipped,price_id,watermark,contacts) values (" .
					$rs->row["id"] . ",'" . pvs_result( $_POST["title"] ) . "','" . $url .
					"'," . ( float )$_POST["price"] . "," . ( int )$_POST["priority"] . ",0," . $id .
					"," . ( int )@$_POST["watermark"] . "," . ( int )@$_POST["contacts"] . ")";
				$db->execute( $sql );
			}
