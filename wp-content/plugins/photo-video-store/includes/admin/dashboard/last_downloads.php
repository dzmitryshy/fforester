...
<div class="postbox" id="box_orders">
	...
	<div class="main">
                  <table class="table no-margin">
                    ...
                    <?php
	$sql = "select id_parent,link,tlimit,ulimit,publication_id,id_parent,user_id,collection_id,package_id from " .
	...
	if ( ! $rs->eof ) {
		...
		?>
					...
					if ( ! $dd->eof ) {
						...
						<tr>
						  <td>
							<a href="<?php echo(pvs_plugins_admin_url('collections/index.php'));?>&action=content&id=<?php echo $rs->row["collection_id"] ?>"><img src="<?php echo $collection_result["photo"]; ?>" style="max-width:<?php echo($pvs_global_settings["thumb_width"]);?>px;max-height:<?php echo($pvs_global_settings["thumb_width"]);?>px"></a>
						  </td>
						  <td>
							<a href="<?php echo(pvs_plugins_admin_url('collections/index.php'));?>&action=content&id=<?php echo $rs->row["collection_id"] ?>" class="product-title"><?php echo pvs_word_lang("Collection") . ": " .  $dd->row["title"] . " (" . pvs_count_files_in_collection($dd->row["id"]) . ")"; ?></a> <span class="label label-warning pull-right"><?php echo pvs_user_id_to_login( $rs->row["user_id"] )?></span>
						  </td>
						</tr>
						<?php					
					}
				...
				if ( ! $dd->eof ) {
					...
					<tr>
					  <td>
						<a href="<?php echo(pvs_plugins_admin_url('packages/index.php'));?>&action=content&id=<?php echo $rs->row["package_id"] ?>"><img src="<?php echo $package_result["photo"]; ?>"  style="max-width:<?php echo($pvs_global_settings["thumb_width"]);?>px;max-height:<?php echo($pvs_global_settings["thumb_width"]);?>px"></a>
					  </td>
					  <td>
						<a href="<?php echo(pvs_plugins_admin_url('packages/index.php'));?>&action=content&id=<?php echo $rs->row["package_id"] ?>" class="product-title"><?php echo pvs_word_lang("package") . ": " .  $dd->row["title"] ; ?></a> <span class="label label-warning pull-right"><?php echo pvs_user_id_to_login( $rs->row["user_id"] )?></span>
					  </td>
					</tr>
					<?php					
				}			
