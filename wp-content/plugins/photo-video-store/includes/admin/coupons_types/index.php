...
//Change
if ( @$_REQUEST["action"] == 'change' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-coupons-change' ) )
{
	include ( "change.php" );
}

//Add
if ( @$_REQUEST["action"] == 'add' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-coupons-add' ) )
{
	include ( "add.php" );
}

//Delete
if ( @$_REQUEST["action"] == 'delete' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-coupons-delete' ) )
{
	include ( "delete.php" );
}
?>
...
?>
	...
	<?php wp_nonce_field( 'pvs-coupons-change' ); ?>
...
?>&action=delete&id=<?php
		echo $rs->row["id_parent"]
?>&_wpnonce=<?php echo wp_create_nonce('pvs-coupons-delete'); ?>' onClick="return confirm('<?php
		echo pvs_word_lang( "delete" )
?>?');"><?php
		echo pvs_word_lang( "delete" )
?></a></div>
...
<form method="post">
	...
	<?php wp_nonce_field( 'pvs-coupons-add' ); ?>
 	<table class="wp-list-table widefat fixed striped posts">
 	<thead>
<tr>
	<th><b><?php
