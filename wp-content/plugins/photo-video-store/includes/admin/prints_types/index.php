...
//Add
if ( @$_REQUEST["action"] == 'add' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-prints-add' )  )
{
	include ( "add.php" );
}

//Change
if ( @$_REQUEST["action"] == 'change' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-prints-change' ) )
{
	include ( "change.php" );
}

//Delete
if ( @$_REQUEST["action"] == 'delete' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-prints-delete' . $_GET["id"] ) )
{
	include ( "delete.php" );
}

//Delete photo
if ( @$_REQUEST["action"] == 'delete_photo'  and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-prints-delete-photo' ) )
{
...
?>:</h1>
	...
	<form method="post">
		...
		<?php wp_nonce_field( 'pvs-prints-change' ); ?>
...
?>&action=delete&id=<?php
			echo $rs->row["id_parent"]
?>&_wpnonce=<?php echo wp_create_nonce('pvs-prints-delete' . $rs->row["id_parent"]); ?>'><?php
...
?>
	...
	<p>2) - To translate the print's names you should add them in /wp-content/plugins/photo-video-store/languages/your-language.mo file.</p>
