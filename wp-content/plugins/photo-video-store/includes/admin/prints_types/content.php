...
<?php wp_nonce_field( 'pvs-prints-add' ); ?>
<?php
if ( isset( $_GET["id"] ) )
{
...
?>
	...
	<script>
		...
		{
			if(jQuery("#resize_print").prop("checked"))
			{
				jQuery('#resize_fields').css("display","block");
			}
			else
			{
				jQuery('#resize_fields').css("display","none");
			}
		}
	</script>
...
?>&type=<?php
		echo $i
?>&_wpnonce=<?php echo wp_create_nonce('pvs-prints-delete-photo'); ?>' class="btn btn-default"><?php
