...
//Delete
if ( @$_REQUEST["action"] == 'delete' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-cart-delete' ) )
{
	include ( "delete.php" );
}

//Remove all
if ( @$_REQUEST["action"] == 'remove_all' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-cart-remove' ) )
{
...
<a class="btn btn-danger toright" href="<?php echo(pvs_plugins_admin_url('shopping_carts/index.php'));?>&action=remove_all&_wpnonce=<?php echo wp_create_nonce('pvs-cart-remove'); ?>"><i class="icon-trash icon-white fa fa-remove"></i> <?php echo pvs_word_lang( "remove all" )?></a>
...
<?php wp_nonce_field( 'pvs-cart-delete' ); ?>
...
<td>
	...
	?></div>
				...
				//Package
				$sql = "select id, title, price, description from " . PVS_DB_PREFIX . "packages where active = 1 and id = " . $dr->row["package"];
				$ds->open( $sql );
				if ( ! $ds->eof ) {
					$price = $ds->row["price"];
					...
					?>
						<div style="margin-bottom:3px"><?php
						echo $title ?> x <?php
						echo $dr->row["quantity"] ?>
