...
//Change
if ( @$_REQUEST["action"] == 'change' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-licenses-change' ) )
{
...
//Insert
if ( @$_REQUEST["action"] == 'insert' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-licenses' ) )
{
...
//Delete
if ( @$_REQUEST["action"] == 'delete' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-licenses-delete' . $_GET["id"] ) )
{
...
?>&action=add&_wpnonce=<?php echo wp_create_nonce('pvs-licenses-add'); ?>"><?php
...
?>
		...
		<?php wp_nonce_field( 'pvs-licenses-change' ); ?>
...
?>&action=delete&id=<?php
			echo $rs->row["id_parent"]
?>&_wpnonce=<?php echo wp_create_nonce('pvs-licenses-delete' . $rs->row["id_parent"]); ?>'><?php
...
?></h1>
	...
	echo ( pvs_build_admin_form( pvs_plugins_admin_url( 'licenses/index.php' ) .
		"&action=insert&id=" . $id, "catalog", "pvs-licenses" ) );
}
?>
