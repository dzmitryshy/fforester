...
//Add
if ( @$_REQUEST["action"] == 'add' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-options-add' ) )
{
	include ( "add.php" );
}

//Change
if ( @$_REQUEST["action"] == 'change' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-options-change' ) )
{
	include ( "change.php" );
}

//Delete
if ( @$_REQUEST["action"] == 'delete' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-options-delete' . $_GET["id"] ) )
{
	include ( "delete.php" );
}
...
?>
		...
		<?php wp_nonce_field( 'pvs-options-change' ); ?>
...
?>&action=delete&id=<?php
			echo $rs->row["id"]
?>&_wpnonce=<?php echo wp_create_nonce('pvs-options-delete' . $rs->row["id"]); ?>'><?php
