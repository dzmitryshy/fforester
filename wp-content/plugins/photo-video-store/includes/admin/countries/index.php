...
//Add
if ( @$_REQUEST["action"] == 'add' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-countries-add' ) )
{
...
//Change
if ( @$_REQUEST["action"] == 'change' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-countries-change' ) )
{
...
//Delete
if ( @$_REQUEST["action"] == 'delete' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-countries-delete' . $_GET["id"] ) )
{
...
{
	jQuery('#form_countries input:checkbox').each(function(){this.checked = !this.checked;});
}

function select_all_eucountries()
{
	jQuery("input:checkbox").attr("checked",false);
	jQuery('input.eu:checkbox').prop('checked','checked');
}
</script>
...
<?php wp_nonce_field( 'pvs-countries-add' ); ?>
...
<?php wp_nonce_field( 'pvs-countries-change' ); ?>
...
?>&action=delete&id=<?php
	echo $rs->row["id"]
?>&_wpnonce=<?php echo wp_create_nonce('pvs-countries-delete' . $rs->row["id"]); ?>'><?php
