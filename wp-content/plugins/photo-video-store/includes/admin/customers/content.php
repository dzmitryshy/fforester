...
function pvs_check_vat(value) 
{
	jQuery.ajax({
		...
		success:function(data){
			if(value == 0)
			{
				jQuery('#vat1').addClass('btn-warning').removeClass('btn-default');
				jQuery('#vat2').removeClass('btn-success').addClass('btn-default');
				jQuery('#vat3').removeClass('btn-danger').addClass('btn-default');
			}
			if(value == 1)
			{
				jQuery('#vat1').removeClass('btn-warning').addClass('btn-default');
				jQuery('#vat2').addClass('btn-success').removeClass('btn-default');
				jQuery('#vat3').removeClass('btn-danger').addClass('btn-default');
			}
			if(value == -1)
			{
				jQuery('#vat1').removeClass('btn-warning').addClass('btn-default');
				jQuery('#vat2').removeClass('btn-success').addClass('btn-default');
				jQuery('#vat3').addClass('btn-danger').removeClass('btn-default');
			}
		}
	});
}
...
function pvs_check_country(value) 
{
	jQuery.ajax({
		...
		success:function(data){
			if(value == 0)
			{
				jQuery('#country1').addClass('btn-warning').removeClass('btn-default');
				jQuery('#country2').removeClass('btn-success').addClass('btn-default');
				jQuery('#country3').removeClass('btn-danger').addClass('btn-default');
			}
			if(value == 1)
			{
				jQuery('#country1').removeClass('btn-warning').addClass('btn-default');
				jQuery('#country2').addClass('btn-success').removeClass('btn-default');
				jQuery('#country3').removeClass('btn-danger').addClass('btn-default');
			}
			if(value == -1)
			{
				jQuery('#country1').removeClass('btn-warning').addClass('btn-default');
				jQuery('#country2').removeClass('btn-success').addClass('btn-default');
				jQuery('#country3').addClass('btn-danger').removeClass('btn-default');
			}
		}
	});
}
...
<?php wp_nonce_field( 'pvs-customers-add' ); ?>
...
if (isset($_GET["id"])) {
	...
	if ( file_exists (pvs_upload_dir() . "/content/users/" . @$user_info -> user_login . ".jpg") ) {
		?>
		<div><a href="<?php echo(pvs_plugins_admin_url('customers/index.php'));?>&action=delete_photo&id=<?php echo((int)$_GET["id"]);?>&_wpnonce=<?php echo wp_create_nonce('pvs-customers-delete-photo'); ?>" class='btn btn-default'><?php echo pvs_word_lang( "delete" );?></a></div>	
		<?php 
	}
}
?>
...
function show_fields(types) {
	if(types=="buyer") {
		...
		jQuery('.payout_field').css("display","none");
	...
	if(types=="seller") {
		...
		document.getElementById('field_aff_commission_buyer').style.display="none";
		document.getElementById('field_aff_commission_seller').style.display="none";
		jQuery('.payout_field').css("display","block");
	}
	if(types=="affiliate") {
		document.getElementById('field_category').style.display="none";
		document.getElementById('field_examination').style.display="none";
