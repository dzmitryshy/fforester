...
//Add
if ( @$_REQUEST["action"] == 'add' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-customers-add' ) )
{
	include ( "add.php" );
}
...
//Delete photo
if ( @$_REQUEST["action"] == 'delete_photo' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-customers-delete-photo' ) )
{
	include ( "delete_photo.php" );
}

//Delete
if ( @$_REQUEST["action"] == 'delete' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-customers-delete' ) )
{
	include ( "delete.php" );
}
...
} else
{
	...
	<?php wp_nonce_field( 'pvs-customers-delete' ); ?>
	...
	<?php
		$property_value = pvs_get_admin_notifications( 'users', 'value' );
		while ( ! $rs->eof ) {
			$cl3 = "";
			$cl_script = "";
