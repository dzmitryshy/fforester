...
if ( $_POST["formaction"] == "delete_category" )
{
	...
	if ( ! isset( $_POST["step"] ) )
	{
		...
		echo ( wp_nonce_field( 'pvs-catalog-edit' ) );
		foreach ( $_POST as $key => $value )
		{
			$res_temp = explode( "sel", $key );
	...
	"thumbs_publication" )
{
	...
	} else
	{
		for ( $i = 0; $i < count( $res_id ); $i++ )
		{
			$sql = "select server1 from " . PVS_DB_PREFIX . "media where media_id=1 and id=" . $res_id[$i];
			$rs->open( $sql );
			if ( ! $rs->eof )
			{
...
if ( $_POST["formaction"] == "change_publication" )
{
	if ( ! isset( $_POST["step"] ) or $_POST["step"] == 1 )
	{
		...
		if ( count( $res_id ) > 0 )
		{
			echo ( "<script>function change_lang_step(){\jQuery('#step_action').val(1);}</script>" );
			...
			//if ( $pvs_global_settings["multilingual_publications"] ) {
				...
				while ( ! $rs->eof )
				{
					...
					{
						if ( $rs->row["name"] == pvs_result($_POST["lang"]) )
						{
							$sel = "selected";
						}
					}
				...
				echo ( "</select>");
				if ( $pvs_global_settings["multilingual_publications"] ) {
					echo("<input type='submit' class='btn btn-primary' value='" . pvs_word_lang( "select" ) . "'>" );
				}
				echo("</div>");
