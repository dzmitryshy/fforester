...
function translation_add(param_field,param_lang,param_lang2,param_lang3,param_type)
{
	...
	jQuery('#trans_'+param_field).append("<div class='clear' id='div_"+param_field+"_"+param_lang3+"' style='padding-top:20px'><div class='input-append' style='float:left;margin-right:4px'>"+input_code+"<span class='add-on' style='width:120px;text-align:left'><img src='<?php
...
function set_expire_date(data_field, value)
{
	if(value == 0)
	{
		jQuery('#ed_' + data_field).slideUp();
	}
	else
	{
		jQuery('#ed_' + data_field).slideDown();
	}
}
</script>
...
echo pvs_build_admin_form( pvs_plugins_admin_url( 'categories/index.php' ) .
	"&action=add&id=" . $id, "category", "pvs-categories-add" )
?>