...
if ( isset( $bigstockphoto_results ) ) {
	...
	$pvs_theme_content[ 'author' ] = '<b>' . pvs_word_lang( "Contributor" ) .
		':</b> <a href="' . pvs_catalog_url(true) . '?stock=bigstockphoto&author=' . @$bigstockphoto_results->
	...
	if ( (int)get_query_var('pvs_print_id') > 0 ) {
		$pvs_theme_content[ 'share_url' ] = urlencode( pvs_print_url( @$bigstockphoto_results->
	...
	if ( (int)get_query_var('pvs_print_id') == 0 ) {
		...
		if ( ! is_wp_error( $result ) ) {
			$bigstockphoto_related = json_decode( $result["body"] );
