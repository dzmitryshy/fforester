...
?>	 	
	 	<?php
	$sql = "select publication_id,link,collection_id,package_id from " . PVS_DB_PREFIX .
		"downloads where user_id=" . get_current_user_id() . " and data>" .
		pvs_get_time( date( "H" ), date( "i" ), date( "s" ), date( "m" ), date( "d" ),
		date( "Y" ) );
	$ds->open( $sql );
...
?>
			<tr>
				...
				if ($ds->row["publication_id"] != 0) {					
					...
					if ($ds->row["collection_id"] != 0) {	
						$collection_result = pvs_show_collection_preview($ds->row["collection_id"]);
						$img_preview = @$collection_result["photo"];
					} else {
						$package_result = pvs_show_package_preview($ds->row["package_id"]);
						$img_preview = @$package_result["photo"];
					}
				}
?>
		<td><div class="profile_home_preview" style="background:url('<?php echo $img_preview; ?>')" onClick="location.href='<?php echo (site_url( ) );?>/download/?f=<?php echo $ds->row["link"]; ?>'"></div></td>
			...
			<?php
				if ($ds->row["publication_id"] != 0) {
					...
					if ($ds->row["collection_id"] != 0) {	
						...
						echo pvs_word_lang('Package') . ' #' . $ds->row["package_id"];
					}
				}
			?>
		</td>
				<td><div class="link_download"><a href="<?php echo (site_url( ) );?>/download/?f=<?php echo $ds->row["link"]; ?>"><?php
...
if ( PVS_LICENSE != 'Free' ) {
	...
	?>
			<td>[<a href="<?php echo (site_url( ) );?>/profile-about/#documents"><?php echo pvs_word_lang( "upload" );?></a>]</td>
		</tr>
		<?php
			$rs->movenext();
		}
