	...
	if ((int)$rs->row["package_id"] == 0) { 
		if ((int)$rs->row["collection_id"] == 0) { 
			...
			if ( ! $dd->eof ) {
				...
				$html_content .= "<tr>";
				$html_content .= "<td>" . pvs_word_lang("Collection") . ": " .  $dd->row["title"] . "</td>";
				$html_content .= "<td>" . date( date_format, ( $rs->row["data"] - $pvs_global_settings["download_expiration"] * 3600 * 24 ) ) . "</td>";
				$html_content .= "<td>Order #" . $rs->row["order_id"] . "</td>";
				$html_content .= "<td>" . pvs_currency( 1, false ) . pvs_price_format( $dd->row["price"], 2 ) . " " . pvs_currency( 2, false ) . "</td>";
				if ( $rs->row["tlimit"] > $rs->row["ulimit"] or $rs->row["data"] < pvs_get_time() ) {
					$html_content .= "<td>Expired</td>";
				} else {
					$html_content .= "<td>" . site_url() . "/download/?f=" . $rs->row["link"] . "</td>";
				}
				$html_content .= "</tr>";
			}		
		}
		...
		if ( ! $dd->eof ) {
			...
			$html_content .= "<tr>";
			$html_content .= "<td>" . pvs_word_lang("Package") . ": " .  $dd->row["title"] . "</td>";
			$html_content .= "<td>" . date( date_format, ( $rs->row["data"] - $pvs_global_settings["download_expiration"] * 3600 * 24 ) ) . "</td>";
			$html_content .= "<td>Order #" . $rs->row["order_id"] . "</td>";
			$html_content .= "<td>" . pvs_currency( 1, false ) . pvs_price_format( $dd->row["price"], 2 ) . " " . pvs_currency( 2, false ) . "</td>";
			if ( $rs->row["tlimit"] > $rs->row["ulimit"] or $rs->row["data"] < pvs_get_time() ) {
