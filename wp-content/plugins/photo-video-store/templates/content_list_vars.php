...
if ( $pvs_global_settings["catalog_url"] != '' ) {
	$redirect_page = $pvs_global_settings["catalog_url"];
}

if ( isset ( $atts[ 'url' ] ) ) {
	$redirect_page = $atts[ 'url' ];
}
...
} else if (get_query_var('pvs_page') == 'category') {
	$_REQUEST["acategory"] = get_query_var('pvs_id');
	$id_parent = get_query_var('pvs_id');
} else {
	$id_parent = get_query_var('pvs_id');
}

if ( isset( $_REQUEST["acategory"] ) and (int)$_REQUEST["acategory"] > 0) {
	$id_parent = (int)$_REQUEST["acategory"];
}

//Stock
if ( isset( $_COOKIE["stock_selected"] ) and $_COOKIE["stock_selected"] != "" and
	@$pvs_global_settings[$_COOKIE["stock_selected"] . "_api"] ) {
	$stock = pvs_result( $_COOKIE["stock_selected"] );
} else
{
...
$flow_vars = pvs_build_variables( "str", "", true, "", false );
...
if ( isset( $_REQUEST["ratio"] ) and $_REQUEST["ratio"] != "" ) {
	$ratio = " and ratio='" . pvs_result( $_REQUEST["ratio"] ) . "'";
}

//Format video
$format = "";
if ( isset( $_REQUEST["format"] ) and $_REQUEST["format"] != "" ) {
	$format = " and format='" . pvs_result( $_REQUEST["format"] ) . "'";
}

//Format audio
$format2 = "";
if ( isset( $_REQUEST["format2"] ) and $_REQUEST["format2"] != "" ) {
	$format2 = " and format='" . pvs_result( $_REQUEST["format2"] ) . "'";
}

//Source
$source = "";
if ( isset( $_REQUEST["source"] ) and $_REQUEST["source"] != "" ) {
	$source = " and source='" . pvs_result( $_REQUEST["source"] ) . "'";
}
...
if ( isset( $_REQUEST["photo_size"] ) ) {
	$photo_size = (int)$_REQUEST["photo_size"];
	if ( $photo_size != 0) {
		$photo_size_sql = " and id in (select id_parent from " .
			PVS_DB_PREFIX . "items where price_id = " . $photo_size . ")";
	}
}
...
$sql = "select id,media_id from " .
	...
	$item_id . $content_type . $publication_date . $holder . $frames . $rendering .
	$ratio . $format . $duration_video . $format2 .
	$source . $duration_audio . $editorial_sql . $adult_sql . $contacts_sql .
	$exclusive_sql . $license_sql . $photo_size_sql . $com2;
...
if ( $stock == 'site' ) {
	if ( $pvs_global_settings["elasticsearch"] and $pvs_global_settings["elasticsearch_search"] and ! isset( $_REQUEST["collection"] ) and ! isset( $_REQUEST["lightbox"] ) and pvs_get_password_protected() == '') {
		...
		$paging_text = pvs_paging( $record_count, $str, $kolvo, $kolvo2, $redirect_page, pvs_build_variables( "str", "", false ), false, $flag_show_end );
	}
}
...
if ( isset( $prints_flag ) and $prints_flag ) {
	$flow = 1;
}
//End. Prints.
