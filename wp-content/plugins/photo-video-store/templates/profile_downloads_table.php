		...
		if ((int)$rs->row["package_id"] == 0) { 
			if ((int)$rs->row["collection_id"] == 0) { 
				...
				if ( $preview != "" ) {
					...
					?>>
			<td><a href="<?php echo pvs_item_url( $rs->row["publication_id"] );?>"><img src="<?php echo $preview; ?>"></a></td>
			<td style="width:30%"><a href="<?php echo pvs_item_url( $rs->row["publication_id"] );?>"><b>#<?php echo $rs->row["publication_id"]; ?></b></a><?php echo $item_name; ?><?php echo $preview_size; ?></td>
			
			<td><?php echo pvs_show_time_ago( $rs->row["data"] - $pvs_global_settings["download_expiration"] * 3600 * 24 );?></td>
			<td>
				<?php
						$price = 0;
			...
			<td>
				<?php
						...
						{
				?>
					<a href="<?php echo site_url(); ?>/download/?f=<?php echo $rs->row["link"]; ?>"><b><?php echo pvs_word_lang( "download" );?></b></a><br>
					<?php echo $rs->row["tlimit"]; ?>(<?php echo $rs->row["ulimit"]; ?>)
					<?php
						}
					?>
			</td>
				...
				if ( ! $dd->eof ) {
					...
					?>
					<tr>
					<td><img src="<?php echo $collection_result["photo"]; ?>"></td>
					<td><a href="<?php echo site_url(); ?>/download/?f=<?php echo $rs->row["link"]; ?>"><b><?php echo pvs_word_lang("Collection"); ?>: <?php echo $dd->row["title"]; ?></b></a> (<?php echo pvs_count_files_in_collection($dd->row["id"]); ?>)</td>
					<td><?php echo pvs_show_time_ago( $rs->row["data"] - $pvs_global_settings["download_expiration"] * 3600 * 24 );?></td>
					<td>
						<?php
						echo ( "<a href='" . site_url() . "/orders_content/?id=" . $rs->row["order_id"] . "'>" . pvs_word_lang( "order" ) . "&nbsp;#" . $rs->row["order_id"] . "</a>" );
						?>
					</td>
					<td><span class="price"><?php
						echo pvs_currency( 1, true );?><?php
						echo pvs_price_format( $dd->row["price"], 2 );?> <?php
						echo pvs_currency( 2, true );?></span></td>
					<td>
					<?php
					if ( $rs->row["tlimit"] > $rs->row["ulimit"] or $rs->row["data"] < pvs_get_time () ) {
						...
						?>
						<a href="<?php echo site_url(); ?>/download/?f=<?php echo $rs->row["link"]; ?>"><b><?php echo pvs_word_lang( "download" );?></b></a><br><?php echo $rs->row["tlimit"]; ?>(<?php echo $rs->row["ulimit"]; ?>)
						<?php
					}
					?>
					</td>
			...
			if ( ! $dd->eof ) {
				...
				?>
				<tr>
				<td><img src="<?php echo $package_result["photo"]; ?>"></td>
				<td><a href="<?php echo site_url(); ?>/download/?f=<?php echo $rs->row["link"]; ?>"><b><?php echo pvs_word_lang("Package"); ?>: <?php echo $dd->row["title"]; ?></b></a></td>
				<td><?php echo pvs_show_time_ago( $rs->row["data"] - $pvs_global_settings["download_expiration"] * 3600 * 24 );?></td>
				<td>
					<?php
					echo ( "<a href='" . site_url() . "/orders_content/?id=" . $rs->row["order_id"] . "'>" . pvs_word_lang( "order" ) . "&nbsp;#" . $rs->row["order_id"] . "</a>" );
