	...
	if ( !isset($collection_id)) {
		...
		if ( pvs_is_remote_storage() ) {
			$sql = "select url,filename1,filename2,width,height,item_id,filesize from " .
				...
				" and  (item_id=" . ( int )$publication_item . " or item_id = -1)";
			$ds->open( $sql );
			while ( ! $ds->eof ) {
		...
		if ( ! $dr->eof ) {
			...
			if ( $publication_type == "photo" ) {
				$sql = "select free,server1,url_jpg,url_png,url_gif,url_raw,url_tiff,url_eps,url_jpf,url_jp2,url_zip,rights_managed from " .
			...
			if ( ! $flag_storage ) {
				if ( $publication_type == "photo" ) {
					...
					{
						$sql = "select jpg,png,gif,raw,tiff,eps,jp2,jpf,zip from " . PVS_DB_PREFIX .
			...
			if ( $publication_type == "photo" ) {
				$sql = "select size,watermark,jpg,png,gif,raw,tiff,eps,jp2,jpf,zip from " . PVS_DB_PREFIX .
					"sizes where id_parent='" . $dr->row["price_id"] . "'";
				$rs->open( $sql );
				if ( ! $rs->eof ) {
					$width = $rs->row["size"];
			...
			if ( $flag_storage ) {
				...
				while ( $file = readdir( $dir ) ) {
					if ( $file == $result_filename . "." . $result_extention )
					{
		...
		if ( ! $ds->eof ) {	
			...
			$sql = "select id,server1,url_jpg,url_png,url_gif,url_raw,url_tiff,url_eps,url_jpf,url_jp2,url_zip,media_id from " . PVS_DB_PREFIX . "media where published = 1 " . $com;
			
			$rs->open($sql);
			while (!$rs->eof) {
				if ($rs->row['media_id'] == 1) {
					$ext_array = array ('jpg', 'png', 'gif', 'raw' , 'tiff',  'eps', 'jp2', 'jpf', 'zip');
			...
			if ( count( $collection_files ) == 0 ) {
				...
				header( "Content-Type:" . $mmtype[strtolower( $result_extention )] );
				header( "Content-Disposition: attachment; filename=" . urlencode( $result_filename ) . "." . $result_extention );
				pvs_readfile_chunked( $result_path );
...
else
{
	...
	$sql = "select id, title, price, description from " . PVS_DB_PREFIX . "packages where active = 1 and id = " . (int)$package_id;

	$ds->open( $sql );
	if ( ! $ds->eof ) {	
		$package_files = array();
		$package_filenames = array();
		
		$result_folder = pvs_upload_dir() . "/content/categories/package-" . $ds->row["id"] . "-" . pvs_result_strict( $_GET["f"] );
		$result_path = $result_folder . ".zip";
		$result_filename = "package-" . $ds->row["id"] . "-" . pvs_result_strict( $_GET["f"] );
		$result_extention = "zip";
		
		$com = '';
		
		$sql = "select id from " . PVS_DB_PREFIX . "orders_content where id_parent=" . (int) $package_order_id;
		$dn->open( $sql );
		while ( ! $dn->eof ) {
			$sql = "select file_id from " . PVS_DB_PREFIX . "packages_list where order_content_id = " . (int) $package_order_content_id  . " and package_id = " . (int)$package_id . " and  file_id > 0 and print_id = 0 and order_content_id=" . $dn->row["id"];
			$rs->open($sql);
			while ( ! $rs->eof) {
				if ( $com != '') {
					$com .= ' or ';
				}
				$com .= ' id = ' . $rs->row["file_id"];
				$rs->movenext();
			}
			$dn->movenext();
		}
		...
		if ( $com != '') {
			...
			while (!$rs->eof) {
				if ($rs->row['media_id'] == 1) {
					...
					for ( $i = 0; $i < count($ext_array); $i++ ) {
						if ($rs->row["url_" . $ext_array[$i]] != '' and file_exists(pvs_upload_dir() . pvs_server_url( $rs->row["server1"] ) . "/" . $rs->row["id"] . "/" . $rs->row["url_" . $ext_array[$i]])) {
							$package_files[$rs->row["id"] . '_'  . $ext_array[$i]] = pvs_upload_dir() . pvs_server_url( $rs->row["server1"] ) . "/" . $rs->row["id"] . "/" . $rs->row["url_" . $ext_array[$i]];
							$package_filenames[$rs->row["id"] . '_'  . $ext_array[$i]] = $rs->row["url_" . $ext_array[$i]];
						} else {
							if ( pvs_is_remote_storage() ) {
								$sql = "select url,filename1,filename2,width,height,item_id,filesize from " . PVS_DB_PREFIX . "filestorage_files where id_parent=" . $rs->row["id"] . " and item_id<>0";
								$dn->open( $sql );
								while ( ! $dn->eof ) {
									...
									$package_files[$rs->row["id"] . '_'  . $ext_array[$i]] = $result_folder . "/" . $rs->row["id"] . "/" . $dn->row["filename1"];
									$package_filenames[$rs->row["id"] . '_'  . $ext_array[$i]] = $dn->row["filename1"];
									$dn->movenext();
								}
							}
						}
					...
					if (!$dr->eof) {
						if (file_exists(pvs_upload_dir() . pvs_server_url( $rs->row["server1"] ) . "/" . $rs->row["id"] . "/" . $dr->row["url"])) {
							$package_files[$rs->row["id"] . '_'  . $dr->row["id"]] = pvs_upload_dir() . pvs_server_url( $rs->row["server1"] ) . "/" . $rs->row["id"] . "/" . $dr->row["url"];
							$package_filenames[$rs->row["id"] . '_'  . $dr->row["id"]] = $dr->row["url"];
						} else {	
							if ( pvs_is_remote_storage() ) {
								$sql = "select url,filename1,filename2,width,height,item_id,filesize from " . PVS_DB_PREFIX . "filestorage_files where id_parent=" . $rs->row["id"] . " and item_id<>0";
								$dn->open( $sql );
								while ( ! $dn->eof ) {	
									...
									$package_files[$rs->row["id"] . '_'  . $dr->row["id"]] = $result_folder . "/" . $rs->row["id"] . "/" . $dn->row["filename1"];
									$package_filenames[$rs->row["id"] . '_'  . $dr->row["id"]] = $dn->row["filename1"];								
									$dn->movenext();
								}
							}
						}
		...
		if ( count( $package_files ) == 0 ) {
			...
			if (! file_exists ($result_folder)) {
				$archive->create( $package_files, PCLZIP_OPT_ADD_PATH, $result_filename, PCLZIP_OPT_REMOVE_PATH, pvs_upload_dir() . "" );
			} else {
				$archive->create( $package_files, PCLZIP_OPT_ADD_PATH, $result_filename, PCLZIP_OPT_REMOVE_PATH, $result_folder );			
			}
