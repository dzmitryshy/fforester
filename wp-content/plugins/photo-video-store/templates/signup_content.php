...
while ( ! $rs->eof ) {
	...
	"address" ) {
		...
		$field .= "<span>" . $bold_begin . pvs_word_lang( "status" ) . $bold_end .
			"</span>
		<select name='business' id='business' style='width:310px' class='ibox form-control'>
		<option value='0' ";
...
function check_state(value) {
    jQuery.ajax({
		...
		data:'country=' + document.getElementById(value).value + '&state=' +  document.getElementById('state').value,
		success:function(data){
			document.getElementById('states_content').innerHTML =data;
		}
	}); 
}
...
function check() {
	...
	if(document.getElementById("company") && document.getElementById("vat") && document.getElementById("business")) {
		if ( document.getElementById("business").value == 1 ) {
			if( document.getElementById("company").value == '' && document.getElementById("vat").value != '' ) {
				jQuery("#company").removeClass("ibox_ok").addClass("ibox_error");
			...
			{
				jQuery("#company").removeClass("ibox_error");
...
function check_login(value) 
{
    jQuery.ajax({
		type:'POST',
		url:'<?php echo (site_url( ) );?>/check-login/',
		data:'login=' + value,
		success:function(data){
			...
			if(data=="<span class='error'><?php echo pvs_word_lang( "Error: Username is already in use." );?></span>" || data=="<span class='error'><?php echo pvs_word_lang( "incorrect field" );?></span>")
			{
				...
				jQuery("#login").removeClass("ibox_ok").addClass("ibox_error");
			}
			else
			{
				document.getElementById('login_response').value=1
				jQuery("#login").removeClass("ibox_error").addClass("ibox_ok");
			}
		}
	});
}
    ...
    jQuery.ajax({
		...
		success:function(data){
			...
			if(data=="<span class='error'><?php echo pvs_word_lang( "Error: Email is already in use." );?></span>" || data=="<span class='error'><?php echo pvs_word_lang( "incorrect field" );?></span>")
			{
				...
				jQuery("#email").addClass("ibox_error").removeClass("ibox_ok");
			}
			else
			{
				document.getElementById('email_ok').value=1
				jQuery("#email").addClass("ibox_ok").removeClass("ibox_error");
			}
		}
	});
}
...
function pvs_check_password() {
	if(document.getElementById('password2').value!="") {
		if(document.getElementById('password').value!=document.getElementById('password2').value) {
			jQuery("#password").addClass("ibox_error").removeClass("ibox_ok");
			jQuery("#password2").addClass("ibox_error").removeClass("ibox_ok");
			document.getElementById('error_password').innerHTML ="<span class='error'><?php echo pvs_word_lang( "not equal" );?></span>";
		}
		else
		{
			jQuery("#password").addClass("ibox_ok").removeClass("ibox_error");
			jQuery("#password2").addClass("ibox_ok").removeClass("ibox_error");
