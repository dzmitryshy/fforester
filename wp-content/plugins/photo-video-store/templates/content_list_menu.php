...
for ( $i = 0; $i < count( $kw_mass ); $i++ ) {
	...
	echo ( "<a href='" . pvs_build_variables( "kw_list", "search" ) . "&kw_list=" . $var_prefix . "'  style='float:right'><i class='fa fa-remove'> </i></a>" );
	echo ( $kw_mass[$i] );
	echo ( "</div>" );
}
...
if ( isset( $_REQUEST["print_id"] ) ) {
	...
	if ( ! $dr->eof ) {
		...
		echo ( "<input type='hidden' name='print_id' value='" . ( int )$_REQUEST["print_id"] .
			"'>" );
		echo ( "<a href='" . $vars_prints . "'  style='float:right'><i class='fa fa-remove'> </i></a>" );
...
if ( ( isset( $_REQUEST["portfolio"] ) and isset( $_REQUEST["user"] ) ) or ( isset
	...
	if ( isset( $_REQUEST["author"] ) and $_REQUEST["author"] != '' ) {
		...
		if ( ! $dr->eof ) {
			echo ( "<div class='search_current_kw'>" );
			echo ( "<a href='" . $vars_author . "'  style='float:right'><i class='fa fa-remove'> </i></a>" );
		...
		if ( isset( $_REQUEST["portfolio"] ) and isset( $_REQUEST["user"] ) ) {
			...
			if ( ! $dr->eof )
			{
				...
				echo ( "<input type='hidden' name='user' value='" . ( int )$_REQUEST["user"] .
					"'>" );
				echo ( "<a href='" . $vars_portfolio . "'  style='float:right'><i class='fa fa-remove'> </i></a>" );
				echo ( $dr->row["user_login"] );
				echo ( "</div>" );
			}
		}
...
if ( isset( $_REQUEST["lightbox"] ) ) {
	...
	if ( ! $dr->eof ) {
		...
		echo ( "<input type='hidden' name='lightbox' value='" . ( int )$_REQUEST["lightbox"] .
			"'>" );
		echo ( "<a href='" . $vars_lightbox . "'  style='float:right'><i class='fa fa-remove'> </i></a>" );
		echo ( $dr->row["title"] );
		echo ( "</div>" );
	}
}
...
if ( isset( $_REQUEST["id_parent"] ) or isset( $_REQUEST["acategory"] ) or isset
	...
	if ( isset( $_REQUEST["category"] ) ) {
		if ( ! preg_match( "/^[0-9]+$/", $_REQUEST["category"] ) ) {
			...
			$sql2 = "select id,title from " . PVS_DB_PREFIX .
				"category where id=" . ( int )$_REQUEST["category"];
		}
	}
	if ( isset( $_REQUEST["id_parent"] ) ) {
	...
	if ( ! $dr->eof ) {
		$translate_results = pvs_translate_category( $dr->row["id"], $dr->row["title"],
			"", "" );
		echo ( "<div class='search_current_kw'>" );
		echo ( "<a href='" . $vars_categories . "'  style='float:right'><i class='fa fa-remove'> </i></a>" );
...
?>
	...
	<div class="search_text">
		<div class="input-group input-group-md">
			<input name="search" type="text" class="form-control" onClick="this.value=''" placeholder="<?php echo(pvs_word_lang("search"));?>">
			<span class="input-group-btn">
			<button class="btn btn-default nobootstrap" style="margin-top:0px;min-width:50px"><i class="fa fa-search"></i></button>
			</span>
		</div>
	</div>
	<script>
...
if ( ! $pvs_global_settings["elasticsearch"] or ! $pvs_global_settings["elasticsearch_search"]) {
	...
	?>
		...
		foreach ( $prints_mass as $key => $value ) {
			...
			while ( ! $dd->eof ) {
				$chk = "";
				if ( isset ( $_REQUEST["print_id"] ) and $_REQUEST["print_id"] == $dd->row["id_parent"] )
				{
					$chk = "selected";
				}
	?>
...
?>
	...
	while ( ! $dr->eof ) {
		$sel = "";
		if ( isset( $_REQUEST["content_type"] ) and $_REQUEST["content_type"] == $dr->row["name"] ) {
			$sel = "selected";
		}
?>
...
?>],
	...
	jQuery( "#price" ).val(jQuery( "#slider-range3" ).slider( "values", 0 ) +
...
?>> <?php echo pvs_word_lang( "creative" )?><br><br>
	<input type="checkbox" name="editorial" <?php echo $editorial
?>> <?php echo pvs_word_lang( "editorial" )?>
			</div>
			
			<div class="search_title2"><?php echo pvs_word_lang( "size" )?>:</div>
			<div class="search_text2">
			<select name="photo_size" class="form-control">
			<option value="0"></option>
			<?php
			$sql2 = "select id_parent from " . PVS_DB_PREFIX . "licenses order by priority";
			$dd->open( $sql2 );
			while( ! $dd->eof ) {
				$sql2 = "select id_parent,title from " . PVS_DB_PREFIX . "sizes where license=" . $dd->row["id_parent"] . " order by priority";
				$dr->open( $sql2 );
				while ( ! $dr->eof )
				{
					$sel="";
					if ( $dr->row["id_parent"] == $photo_size) {
						$sel = "selected";
					}
					?>
					<option value="<?php echo($dr->row["id_parent"]);?>" <?php echo($sel);?>><?php echo($dr->row["title"]);?></option>
					<?php
					$dr->movenext();
				}
				$dd->movenext();
...
?>> <?php echo pvs_word_lang( "all" )?><br><br>
	<input type="radio" name="orientation" value="0" <?php
	if ( $default_orientation == 0 ) {
		echo ( "checked" );
	}
?>> <?php echo pvs_word_lang( "landscape" )?><br><br>
	<input type="radio" name="orientation" value="1" <?php
	if ( $default_orientation == 1 ) {
		echo ( "checked" );
	}
...
?>],
		slide: function( event, ui ) {
		jQuery( "#duration_video" ).val(ui.values[ 0 ] + " - " + ui.values[ 1 ] );
		}
		});
		jQuery( "#duration_video" ).val(jQuery( "#slider-range" ).slider( "values", 0 ) +
...
?>],
	slide: function( event, ui ) {
	jQuery( "#duration_audio" ).val(ui.values[ 0 ] + " - " + ui.values[ 1 ] );
	}
	});
	jQuery( "#duration_audio" ).val(jQuery( "#slider-range2" ).slider( "values", 0 ) +
	" - " + jQuery( "#slider-range2" ).slider( "values", 1 ) );
	});
	</script>
	<div class="box_slider">
...
?>
    ...
    jQuery("#search_left_categories_menu").navgoco({
        caretHtml: '',
        accordion: false,
        openClass: 'open',
        save: true,
   ...
   // jQuery("#search_left_categories_menu").navgoco('toggle', true,0,10,17);
