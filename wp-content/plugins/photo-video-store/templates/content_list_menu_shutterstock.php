...
<script>
	...
	function activate_fields(value) {
		...
		for(i=0;i<fields_stock.length;i++) {
			jQuery('.field_' + fields_stock[i]).css('display','block');
		}
	}
	...
	function deactivate_fields(value) {
		...
		for(i=0;i<fields_stock.length;i++) {
			jQuery('.field_' + fields_stock[i]).css('display','none');
		}
	}
</script>
...
?>
	...
	<script>jQuery('#colorSelector2').ColorPicker({
...
?>],
			slide: function( event, ui ) {
			jQuery( "#bmp" ).val(ui.values[ 0 ] + " - " + ui.values[ 1 ] );
			}
			});
			jQuery( "#bmp" ).val(jQuery( "#slider-range2" ).slider( "values", 0 ) +
			" - " + jQuery( "#slider-range2" ).slider( "values", 1 ) );
			});
			</script>
			<div class="box_slider">
