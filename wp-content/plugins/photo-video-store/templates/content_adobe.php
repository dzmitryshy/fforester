...
if ( isset( $adobe_results ) ) {
	...
	if ( ! $prints_flag ) {
		...
		if ( $pvs_global_settings["adobe_prints"] and $publication_type != "video" )
		{
			...
			$sizes .= "</table><br><a href=\"javascript:prints_stock('adobe'," . @$adobe_results->
				id . ",'" . urlencode( $adobe_results->details_url ) . "','" . urlencode( @$adobe_results->
				assets->thumbnail_url ) . "','" . pvs_get_stock_page_url( "adobe", @$adobe_results->
	...
	if ( (int)get_query_var('pvs_print_id') > 0 ) {
		...
		$iframe_width = @$adobe_results->width;
		$iframe_height = @$adobe_results->height;
		...
		$pvs_theme_content[ 'stock_url' ] = pvs_get_stock_affiliate_url( "adobe", @$adobe_results->id, "stock" );
