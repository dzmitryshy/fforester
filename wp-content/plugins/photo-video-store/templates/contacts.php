...
//Send
if ( @$_REQUEST["action"] == 'send'  and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-contacts-send' ) )
{
	//Check captcha
...
if ( isset( $_GET["file_id"] ) ) {
	$question = pvs_word_lang( "I would like to purchase the file" ) . " #" . ( int ) $_GET["file_id"];
	if ( isset( $_GET["item"] ) ) {
		$sql="select name from " . PVS_DB_PREFIX . "items where id=" . (int) $_GET["item"];
		$rs->open($sql);
		if ( ! $rs->eof ) {
			$question .= "\n" . $rs->row["name"];
		}
...
<?php wp_nonce_field( 'pvs-contacts-send' ); ?>
