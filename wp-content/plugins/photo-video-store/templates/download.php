...
if ( isset( $_GET["f"] ) ) {
	$sql = "select id_parent,link,data,tlimit,ulimit,publication_id,collection_id,package_id,order_id from " .
	...
	if ( ! $ds->eof ) {
		if ( (int) $ds->row["package_id"] == 0) {
			...
			$sql = "select id, title, price, description from " . PVS_DB_PREFIX . "packages where active = 1 and id = " . (int)$ds->row["package_id"];
			$dd->open( $sql );
			if ( ! $dd->eof ) {
				$download_regime = "package";
				$package_id = (int)$ds->row["package_id"];
				$package_order_id = (int)$ds->row["order_id"];
				$package_order_content_id = (int)$ds->row["id_parent"];
				include ( "download_process.php" );
			}		
		}
	} else {
