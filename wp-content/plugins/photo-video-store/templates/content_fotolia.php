...
if ( isset( $fotolia_results ) ) {
	...
	$pvs_theme_content[ 'author' ] = '<b>' . pvs_word_lang( "Contributor" ) .
		':</b> <a href="' . pvs_catalog_url(true) . '?stock=fotolia&author=' . $fotolia_results->
	...
	if ( (int)get_query_var('pvs_print_id') > 0 ) {
		$pvs_theme_content[ 'share_url' ] = urlencode( pvs_print_url( @$fotolia_results->
	...
	if ( (int)get_query_var('pvs_print_id') == 0 ) {
		if ( get_query_var("fotolia_type") != "audio" )
		{
			...
			if ( ! is_wp_error( $result ) ) {
				$fotolia_related = json_decode( $result["body"] );
