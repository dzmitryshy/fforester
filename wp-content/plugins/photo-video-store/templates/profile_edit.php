...
if ( ! wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-signup' ) ) {	
	exit();
}
...
if ( $flag == true and @$_POST["password"] != '') {
	...
	if ( pvs_result( @$_POST["vat"] ) != '' and $pvs_global_settings["eu_tax"] and $pvs_global_settings["eu_tax_vat_validation"]) {
		pvs_check_vat(pvs_result( @$_POST["vat"] ), $id);
	}
