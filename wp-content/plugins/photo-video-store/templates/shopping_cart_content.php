					...
					<?php
							...
							while ( ! $ds->eof )
							{
								...
								while ( ! $dd->eof )
								{
									...
									$dn->open( $sql );
									if ( ! $dn->eof and $flag_size and (int)$dn->row["contacts"] == 0 )
									{
										$chk = "";
		...
		?>);"><?php
			...
			if ( ! $ds->eof ) {
				...
				<tr valign="top">
					<td>
					<?php
						$package_result = pvs_show_package_preview($ds->row["id"]);
						$url = pvs_package_url($ds->row["id"], $dq->row["id"]);
					?>
					<a href="<?php echo($url);?>"><img src="<?php echo($package_result["photo"]);?>" style="max-width:<?php echo($pvs_global_settings["thumb_width"]);?>px;max-height:<?php echo($pvs_global_settings["thumb_width"]);?>px"></a>
					</td>
					<td><a href="<?php echo($url);?>"><?php echo(pvs_word_lang("Package"));?>: <?php echo($ds->row["title"])?></a><br><?php echo(pvs_count_files_in_package($ds->row["id"], $dq->row["id"]));?><br><?php echo($ds->row["description"]);?></td>
					<td>
					<span class="price"><b><?php
						echo pvs_currency( 1 );?><?php
						echo pvs_price_format( $ds->row["price"], 2, true );?> <?php
