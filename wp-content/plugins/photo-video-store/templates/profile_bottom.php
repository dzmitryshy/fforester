...
?>
		...
		function get_clarifai(url,field_id,language,default_value) {
			...
			if(jQuery("#recognition_lang").length>0)
			{
				language = jQuery("#recognition_lang").val();
				if(default_value)
				{
					language = site_lang_symbol[jQuery("#recognition_lang").val()];
				}
			}
			...
			app.models.predict(Clarifai.GENERAL_MODEL, url, {video:video_flag}, {language: lang_to_lang[language]})
 			 .then(
				...
				if(jQuery("#"+field_id+"_box"))
				{
					jQuery("#"+field_id+"_box").css("display","block");
				}
				jQuery("#"+field_id).val(tags);
			  },
			  function(err) {
			  	//jQuery("#"+field_id+"_box").css("display","block");
				//jQuery("#"+field_id).val(err);
			  }
			);
...
?>
	...
	function get_imagga(url,field_id,language,default_value) 
	{
		...
		jQuery.ajax({
			type:'POST',
			url:'<?php echo (site_url( ) );?>/recognition-imagga/',
			data:'url=' + url + '&lang=' + lang_to_lang[language],
...
?>
	...
	function apply_keywords(from_field, to_field) {
		if(jQuery("#recognition_lang")) {
			language = jQuery("#recognition_lang").val();
		}
		
		keywords = jQuery("#"+from_field).val();
		keywords_mass = keywords.split(',');
		
		keywords2 = jQuery("#"+to_field).val();
		...
		jQuery("#"+to_field).val(tags);
		jQuery("#"+from_field+"_box").css("display","none");
		
		change_group('other') ;
		
		jQuery('html, body').animate({
        	scrollTop: jQuery("#"+to_field).offset().top
    	}, 2000);
	}
