...
if ( isset( $istockphoto_results ) ) {
	...
	$pvs_theme_content[ 'author' ] = '<b>' . pvs_word_lang( "Artist" ) .
		':</b> <a href="' . pvs_catalog_url(true) . '?stock=istockphoto&author=' . @$istockphoto_results->
	...
	if ( (int)get_query_var('pvs_print_id') > 0 ) {
		$pvs_theme_content[ 'share_url' ] = urlencode( pvs_print_url( @$istockphoto_results->
	...
	if ( (int)get_query_var('pvs_print_id') == 0 ) {
		if ( get_query_var('istockphoto_type') == "photo" )
		{
			...
			$sizes .= "<button onClick='location.href=\"" . pvs_get_stock_affiliate_url( "istockphoto", @$istockphoto_results->
				id, get_query_var('istockphoto_type'), @$referal_url[0]->uri, @$referal_url[1]->uri ) .
				"\"' target='blank' class = 'btn btn-primary btn-lg' style='text-decoration:none'>" .
			...
			if ( $pvs_global_settings["istockphoto_prints"] )
			{
				...
				$sizes .= "</table><br><button onClick=\"prints_stock('istockphoto'," . @$istockphoto_results->
					...
					"','" . addslashes( @$istockphoto_results->title ) . "')\" class = 'btn btn-danger btn-lg' style='text-decoration:none;'>" .
					pvs_word_lang( "Order print" ) . "</button></div>";
			}
		} else
		{
	...
	if ( (int)get_query_var('pvs_print_id') == 0 ) {
		...
		if ( ! is_wp_error( $result ) ) {
			$istockphoto_related = json_decode( $result["body"] );
