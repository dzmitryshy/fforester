...
if ( pvs_check_password_publication( get_query_var('pvs_id') ) ) {
	...
	$sql = "select id,media_id,title,data,description,viewed,author,keywords,userid,usa,duration,format,ratio,rendering,holder,frames,content_type,free,downloaded,rating,server1,google_x,google_y,url,rights_managed,vote_like,vote_dislike,contacts,exclusive,content_type,editorial from " .
	...
	if ( ! $rs->eof ) {
		...
		if ( file_exists ( get_stylesheet_directory(). '/item_video.php' ) ) {
			require_once( get_stylesheet_directory(). '/item_video.php' );
		} else {
			if ( file_exists ( PVS_PATH . 'templates/item_video.php' ) ) {
				require_once( PVS_PATH . 'templates/item_video.php' );
			}
		}
	}
}
