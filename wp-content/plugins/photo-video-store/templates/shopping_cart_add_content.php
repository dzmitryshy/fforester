		...
		if ( (int) $rs->row["package"] == 0 ) {
			if ( (int) $rs->row["collection"] == 0 ) {
				if ( $rs->row["item_id"] > 0 )
				{
					...
					if ( ! $dr->eof )
					{
						$sql = "select id,media_id,title,server1,url_jpg,url_png,url_gif,url_raw,url_tiff,url_eps,url_jpf,url_jp2,url_zip from " .
					...
					if ( $rs->row["rights_managed"] != "" )
					{
						...
						$script_carts_price .= pvs_price_format($rights_managed_price,2);
						$script_carts_url .= "'" . pvs_item_url( $dr->row["id_parent"] ) . "'";
						$script_carts_price2 .= pvs_price_format($rights_managed_price,2);
					...
					} else
					{
						...
						if ( ! $dr->eof )
						{
							...
							$script_carts_price .= pvs_price_format($dr->row["price"],2);
							$script_carts_url .= "'" . pvs_item_url( $dr->row["id_parent"] ) . "'";
					...
					if ( ( int )$rs->row["stock"] == 0 )
					{
						if ( $rs->row["printslab"] <> 1 )
						{
							...
							if ( ! $dr->eof )
							{
								...
								$script_carts_price .= pvs_price_format($price,2);
	
								$script_carts_price2 .= pvs_price_format($price,2);
			...
			//Package
			$sql = "select id, title, price, description from " . PVS_DB_PREFIX . "packages where active = 1 and id = " . $rs->row["package"];
			$ds->open( $sql );
			if ( ! $ds->eof ) {
				$price = $ds->row["price"];

				$box_shopping_cart .=
					"<tr><td class='tcontent'><span class='smalltext'><a href=''>" . $ds->row["title"] .
					"</a></td><td  class=tcontent></td><td  class=tcontent><span class=smalltext><span class='price'>" .
					pvs_price_format( $price, 2, true ) .
					"</span></td><td class=tcontent><span class=smalltext>" . $rs->row["quantity"] .
					"</td></tr>";
				...
				$collection_result = pvs_show_package_preview($ds->row["id"]);

				$title = pvs_word_lang("Package") . ': ' . $ds->row["title"];
				$preview = $collection_result["photo"];
				$url =pvs_package_url($ds->row["id"], $rs->row["id"]);
